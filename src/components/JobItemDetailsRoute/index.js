...
import SimilarJobBlocks from '../SimilarJobBlocks'
      ...
      <div className="full-job-item-container">
        ...
        <ul className="similar-cards">
          {similarJobItemList.map(eachItem => (
            <SimilarJobBlocks key={eachItem.id} jobDetails={eachItem} />
          ))}
        </ul>
      </div>
    )
...
Job Item Details Route
    ...
    When an authenticated user opens the Job Item Details Route
        1.An HTTP GET request should be made to jobDetailsApiUrl with jwt_token
        in the Cookies and job id as path parameter
            2.loader should be displayed while fetching the data
            3.After the data is fetched successfully, the response received should be displayed
            4.The list of similar jobs should be displayed
            5.If the HTTP GET request made is unsuccessful, then the Failure View should be displayed
               6. When the Retry button is clicked, an HTTP GET request should be made to jobDetailsApiUrl
